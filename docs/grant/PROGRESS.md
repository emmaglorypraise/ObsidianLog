...
| Deliverable / task | Pull Request(s) / Commit(s) | Status / Notes |
| --- | --- | --- |
| GitHub Actions reusable workflow | [#65](https://github.com/emmaglorypraise/ObsidianLog/pull/65) | _Done._ `.github/workflows/demo.yml` runs the full pipeline against the local backend, no secrets needed. Forkable, and runs on every push to `main`. |
| Documentation site | [#66](https://github.com/emmaglorypraise/ObsidianLog/pull/66), [#76](https://github.com/emmaglorypraise/ObsidianLog/pull/76), [#86](https://github.com/emmaglorypraise/ObsidianLog/pull/86), [#87](https://github.com/emmaglorypraise/ObsidianLog/pull/87), [#88](https://github.com/emmaglorypraise/ObsidianLog/pull/88) | _Done._ Mintlify docs site, live at [docs.obsidianlog.com](https://docs.obsidianlog.com/). Includes a Tutorials section (four hands-on walkthroughs) and a beta-launch landing page, supporting the "10 external developers tested it" success criterion below. |
| Live end-to-end demo | _pending_ | A screen-recorded video, distinct from the GitHub Actions demo above. |
| Indexer topology: hosted-default + bring-your-own-indexer | [#44](https://github.com/emmaglorypraise/ObsidianLog/pull/44), [#63](https://github.com/emmaglorypraise/ObsidianLog/pull/63) | _Done._ [ADR-0007](../adr/0007-indexer-topology.md): "hosted" defaults `obsidianlog init` to [sia.storage](https://sia.storage) (self-hosting `indexd` remains a documented option). Verified end-to-end against real sia.storage. |
| Close the ADR-0009 residual gap: refuse ingest on a lost/stale manifest with existing chunks | [#68](https://github.com/emmaglorypraise/ObsidianLog/pull/68) | _Done._ Raised by the Month 2 reviewer. [ADR-0013](../adr/0013-refuse-ingest-on-lost-manifest-with-existing-chunks.md): refuses to ingest rather than silently restarting a service's nonce counter after manifest loss. |
| Enforce the single-writer assumption per data directory (advisory lock) | [#70](https://github.com/emmaglorypraise/ObsidianLog/pull/70) | _Done._ Raised by the Month 2 reviewer. [ADR-0014](../adr/0014-single-writer-advisory-lock.md): `obsidianlog serve` takes an exclusive OS advisory lock on the data directory, so a second instance fails fast instead of racing on the manifest. |
| Fix: `obsidianlog init`'s repair path silently rotated the key without `--force` | [#72](https://github.com/emmaglorypraise/ObsidianLog/pull/72) | _Done._ Reported by the Month 2 reviewer. `finish_init` now only generates a key when none exists yet or `--force` is passed. |
| Further reduce `obsidianlog init`'s OS keychain prompts to one | [#80](https://github.com/emmaglorypraise/ObsidianLog/pull/80), [#82](https://github.com/emmaglorypraise/ObsidianLog/pull/82) | _Done._ [ADR-0015](../adr/0015-bundled-credential-single-prompt-setup.md) redesigns local credential storage around one bundled keychain item, bringing a fresh install or repair down to a single prompt. Breaking config/keychain format change, bumped to [`0.2.0`](https://github.com/emmaglorypraise/ObsidianLog/releases/tag/v0.2.0). Verified end-to-end on real macOS hardware. |
| Ship the Sia backend in the official release binaries | [#81](https://github.com/emmaglorypraise/ObsidianLog/pull/81) | _Done._ Release binaries now build `obsidianlog` with `--features sia` for all 5 targets, so testers get Sia support without a source build. Verified with a real onboarding → archive → query → verify run from a downloaded artifact. |
| Example integrations: Grafana + SIEM export workflows | [#67](https://github.com/emmaglorypraise/ObsidianLog/pull/67) | _Done._ `examples/integrations/grafana/` and `examples/integrations/siem/` — worked examples fed by `obsidianlog query`, with docs pages under `docs-site/integrations/`. |
| Real Windows fixes from real tester feedback | [#98](https://github.com/emmaglorypraise/ObsidianLog/pull/98) | _Done._ Fixed two real Windows bugs from external testers (config-path fallback, PowerShell `curl.exe` quoting) and a security bug in the Grafana example. Extended Windows coverage across the docs. Evidences the "10 external developers tested it" success criterion below — see issues [#95](https://github.com/emmaglorypraise/ObsidianLog/issues/95), [#96](https://github.com/emmaglorypraise/ObsidianLog/issues/96), [#97](https://github.com/emmaglorypraise/ObsidianLog/issues/97), [#100](https://github.com/emmaglorypraise/ObsidianLog/issues/100). |
| Published to crates.io | [#101](https://github.com/emmaglorypraise/ObsidianLog/pull/101), [#103](https://github.com/emmaglorypraise/ObsidianLog/pull/103) | _Done._ Additional reach beyond the original deliverables. All four crates published at `0.2.0`: [core](https://crates.io/crates/obsidianlog-core), [store](https://crates.io/crates/obsidianlog-store), [ingest](https://crates.io/crates/obsidianlog-ingest), [cli](https://crates.io/crates/obsidianlog-cli). Docs updated with real `cargo install` instructions. |
