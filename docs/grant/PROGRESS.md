...
| Fix: Docker quickstart published `/ingest` on all host network interfaces | [#49](https://github.com/emmaglorypraise/ObsidianLog/pull/49) | _Done._ Found via manual testing of #48. `/ingest` has no request authentication and relies entirely on its `bind = 127.0.0.1` default for protection (see ADR-0012), but the compose file published the port as `"7080:7080"` — reachable from any host on the network, not just loopback — silently defeating that default. Restricted to `"127.0.0.1:7080:7080"`, matching the pattern already used for `indexd`'s admin port in the same file. ADR-0012 records the decision to rely on network isolation rather than add request auth, and the trigger for revisiting that (a real multi-host deployment need). |
