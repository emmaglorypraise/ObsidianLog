...
| crates.io publish readiness | [#101](https://github.com/emmaglorypraise/ObsidianLog/pull/101) | _In progress._ Not one of the original Month 3 deliverables, pursued as additional reach. Fixed a docs.rs metadata bug (`obsidianlog-cli`'s `documentation` field pointed at the wrong crate name). Internal workspace dependencies already carry both `path` and `version`, so the four crates are ready to publish in dependency order (`core` → `store` → `ingest` → `cli`); the first real `cargo publish` hasn't been run yet. |
