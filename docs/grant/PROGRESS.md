...
| Further reduce `obsidianlog init`'s OS keychain prompts to one | [#80](https://github.com/emmaglorypraise/ObsidianLog/pull/80) | _Done._ Builds on [#58](https://github.com/emmaglorypraise/ObsidianLog/pull/58) (Month 2): trimming redundant calls alone could never reach one prompt, since the design still checked for, then separately wrote, two independent keychain items. [ADR-0015](../adr/0015-bundled-credential-single-prompt-setup.md) redesigns local credential storage around one bundled item (encryption key + optional Sia app key) with a macOS-specific "create only" write, bringing a fresh install (local or Sia) and a plain config-only repair down to a single keychain prompt. Breaking change to the config and keychain formats — a pre-0.2 config fails to load with a clear message, no automatic migration — bumped to `0.2.0` (not yet released). **Verified end-to-end on real macOS hardware**: fresh local and fresh Sia installs confirmed at no more than one keychain prompt against a cleared keychain; a config-only repair proven — with real ingested data, not just unit tests — to preserve the encryption key rather than rotate it, and now prints a status line explaining why the wizard re-ran; forced rotation reduced from three keychain operations to two after a redundant preflight read was caught during verification; a Sia onboarding pitfall (a mistyped `seed` keyword silently burning a full browser-approval round-trip, and an indexer app-connection slot, before failing) fixed by validating the recovery phrase locally first. |
