...
| Documentation site | [#66](https://github.com/emmaglorypraise/ObsidianLog/pull/66), [#76](https://github.com/emmaglorypraise/ObsidianLog/pull/76), [#86](https://github.com/emmaglorypraise/ObsidianLog/pull/86), [#87](https://github.com/emmaglorypraise/ObsidianLog/pull/87), [#88](https://github.com/emmaglorypraise/ObsidianLog/pull/88) | _Done._ `docs-site/` — Mintlify site (Get Started, Tutorials, CLI reference, Storage Backends, Deployment, Integrations, Architecture & Security, Project), verified with a real local `mintlify dev` preview and `mintlify broken-links`. Hosting connected and live at [docs.obsidianlog.com](https://docs.obsidianlog.com/) (custom domain, previously `obsidianlog.mintlify.site`), linked from the README. #86–#88 add a new Tutorials section: four hands-on walkthroughs (local backend, Sia backend, returning user, Vector integration) run against real release binaries, a dedicated beta-launch landing page (`/beta`) and release-file-selection FAQ, and a sitewide plain-English/we-us-voice readability pass — directly supporting the "10 external developers tested it" outreach success criterion below. |
...
| Further reduce `obsidianlog init`'s OS keychain prompts to one | [#80](https://github.com/emmaglorypraise/ObsidianLog/pull/80), [#82](https://github.com/emmaglorypraise/ObsidianLog/pull/82) | _Done._ Builds on [#58](https://github.com/emmaglorypraise/ObsidianLog/pull/58) (Month 2): trimming redundant calls alone could never reach one prompt, since the design still checked for, then separately wrote, two independent keychain items. [ADR-0015](../adr/0015-bundled-credential-single-prompt-setup.md) redesigns local credential storage around one bundled item (encryption key + optional Sia app key) with a macOS-specific "create only" write, bringing a fresh install (local or Sia) and a plain config-only repair down to a single keychain prompt. Breaking change to the config and keychain formats — a pre-0.2 config fails to load with a clear message, no automatic migration — bumped to [`0.2.0`](https://github.com/emmaglorypraise/ObsidianLog/releases/tag/v0.2.0). **Verified end-to-end on real macOS hardware**: fresh local and fresh Sia installs confirmed at no more than one keychain prompt against a cleared keychain; a config-only repair proven — with real ingested data, not just unit tests — to preserve the encryption key rather than rotate it, and now prints a status line explaining why the wizard re-ran; forced rotation reduced from three keychain operations to two after a redundant preflight read was caught during verification; a Sia onboarding pitfall (a mistyped `seed` keyword silently burning a full browser-approval round-trip, and an indexer app-connection slot, before failing) fixed by validating the recovery phrase locally first. Follow-up (#82): the same reconfiguration path (existing setup found, reuse declined, e.g. to add Sia) still re-probed the keychain with a `create()` attempt that could only report `AlreadyExists` and a second `read()` for a bundle already read once during the integrity check — now reuses that known bundle directly, cutting this case from up to four keychain operations down to at most two; also corrected a confirmation prompt that inaccurately implied declining reuse would rotate the encryption key. |
| Ship the Sia backend in the official release binaries | [#81](https://github.com/emmaglorypraise/ObsidianLog/pull/81) | _Done._ Release binaries previously shipped `obsidianlog` without the `sia` Cargo feature, pointing testers at a source build for the Foundation-sponsored storage path itself. The release matrix now builds `obsidianlog` with `--features sia` for all 5 targets; `obsidianlog init` still defaults to the local backend regardless — this only changes what's compiled in. **Verified for real**: a throwaway `v0.2.0-rc.1` tag confirmed all 5 targets (including the two musl cross-compiles, the main cross-compilation risk given `sia`'s `ring` dependency) build successfully and pass the existing static-link checks, and a genuine Sia onboarding → archive → query → verify run from the downloaded macOS artifact succeeded (~4.6 minutes first-write latency for real Sia contract/storage provisioning, consistent with the earlier hosted-indexer verification). The rc tag/release were deleted once confirmed, mirroring the `v0.1.0-rc.1`/`rc.2` precedent. `obsidianlog-ingest` (the standalone, local-only binary) is unaffected — it has no `sia` feature to enable. |
...
| Real Windows fixes from real tester feedback | [#98](https://github.com/emmaglorypraise/ObsidianLog/pull/98) | _Done._ Two genuine bugs surfaced by external testers actually running the released Windows binary, not internal testing: [#96](https://github.com/emmaglorypraise/ObsidianLog/issues/96) (no fallback in the default config-path resolution when neither `XDG_CONFIG_HOME` nor `HOME` is set — the default state of any Windows install) and a PowerShell quoting issue affecting the documented `curl.exe` ingest command. Also fixed a real security bug found while reviewing the Grafana integration example: `export-and-serve.sh` didn't actually bind to localhost as the docs claimed, exposing decrypted log data to the network by default. Extended Windows command coverage across Quickstart, all four tutorials, Docker Compose, and the Grafana/SIEM integration pages; reorganized the docs nav. Directly evidences the "10 external developers tested it" success criterion below — see [#95](https://github.com/emmaglorypraise/ObsidianLog/issues/95), [#96](https://github.com/emmaglorypraise/ObsidianLog/issues/96), [#97](https://github.com/emmaglorypraise/ObsidianLog/issues/97), and [#100](https://github.com/emmaglorypraise/ObsidianLog/issues/100) for the underlying tester reports. |
| Published to crates.io | [#101](https://github.com/emmaglorypraise/ObsidianLog/pull/101), [#103](https://github.com/emmaglorypraise/ObsidianLog/pull/103) | _Done._ Not one of the original Month 3 deliverables, pursued as additional reach. Fixed a docs.rs metadata bug and a missing-readme gap (`obsidianlog-core`, `-store`, and `-ingest` had no `readme` field; discovered after publishing `core` and its crates.io page rendered empty). All four crates published at `0.2.0`, in dependency order (`core` → `store` → `ingest` → `cli`): [`obsidianlog-core`](https://crates.io/crates/obsidianlog-core), [`obsidianlog-store`](https://crates.io/crates/obsidianlog-store), [`obsidianlog-ingest`](https://crates.io/crates/obsidianlog-ingest), [`obsidianlog-cli`](https://crates.io/crates/obsidianlog-cli). |
