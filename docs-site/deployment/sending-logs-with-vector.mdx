...
This page is the config reference for a real, ongoing deployment. Want to
see it proven end to end first, a scratch test file, a real Vector run,
and the archived data queried back, before wiring up a production
config? See [Trying Out Vector](/tutorials/vector-integration).

## Example configuration

```toml
# Where Vector persists file-source checkpoint state (which lines it's
# already read). Without this, Vector falls back to /var/lib/vector/, a
# Unix path that doesn't exist on Windows, and refuses to start. Vector
# doesn't create this directory itself, run `mkdir vector-data` once
...
<Tabs>
  ...
  <Tab title="Windows">
    ```powershell
    # Run this after `obsidianlog init`. Use .\obsidianlog.exe if you are running
    # directly from an extracted release archive rather than installing it on PATH.
