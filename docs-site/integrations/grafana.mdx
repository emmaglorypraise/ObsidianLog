...
<Note>
This is an example, not a built-in feature. A dedicated Grafana datasource
plugin is a possible future direction, not something ObsidianLog includes
today.
</Note>
...
script exports a query to JSON and serves it over HTTP. This needs Python
installed (Windows doesn't bundle it the way macOS and most Linux
distributions do, get it from [python.org](https://www.python.org/downloads/)
...
<Tabs>
  ...
  <Tab title="Windows">
    ```powershell
    .\obsidianlog.exe query --format json > logs.json
    python -m http.server 8787 --bind 127.0.0.1   # serves logs.json at :8787/logs.json
    ```
  </Tab>
</Tabs>
...
in the repo. It's a bash script (macOS/Linux, or WSL/Git Bash on
Windows). On Windows without one of those, run the two commands above
directly instead, adjusting the port or adding `--config <path>` to the
query command as needed.
...
3. Run the export script (leave it running), or the two commands from
   [How it works](#how-it-works) above on Windows without WSL/Git Bash:
   ```sh
