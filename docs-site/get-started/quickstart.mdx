...
<Tabs>
  ...
  <Tab title="Windows">
    ...
    ObsidianLog looks for by default, so pass `--config` explicitly every
    time to sidestep it entirely:
    ```powershell
    $config = "$env:USERPROFILE\.obsidianlog\config.toml"
    ```

    **1. Set up.**
    ```powershell
    .\obsidianlog.exe init --non-interactive --config $config
    ```
    You'll see:
    ```text
    obsidianlog initialized.
      config:      C:\Users\you\.obsidianlog\config.toml
      credentials: the OS keychain
    ```

    **2. Start the server.**
    ```powershell
    .\obsidianlog.exe serve --config $config
    ```
    The terminal won't return to a prompt after this, it keeps running
    until you stop it. That's expected, leave it running and use your
    second terminal for the rest.

    **3. In the second terminal, send a log.** Assign the JSON to a
    variable first, PowerShell's quoting breaks on a curl command written
    the Unix way:
    ```powershell
    ...
    **4. Query it back.**
    ```powershell
    .\obsidianlog.exe query --config $config --service api --level info --from 24h --format human
    ```
    ```text
    2026-07-06T10:00:00Z  api  info  -  hello
    ```
    ...
    **5. Verify the hash chain.**
    ```powershell
    .\obsidianlog.exe verify --config $config
    ```
    ```text
    OK   api: 1 chunk(s) verified, chain intact
    ```
