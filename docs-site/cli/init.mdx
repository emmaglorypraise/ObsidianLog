...
| `--force` | Overwrites an existing configuration. This rotates the encryption key, which makes previously archived data undecryptable with the new key. `--force` is explicit opt-in, so it prints a warning and proceeds without asking for confirmation. |
